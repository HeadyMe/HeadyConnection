...
if ($GenerateKeys) {
    ...
    foreach ($service in $services) {
        ...
        if ($key) {
            try {
                ...
                Write-Host "❌ Failed to add $($service.name) key: Error occurred" -ForegroundColor Red
            }
        }
    }
    ...
    try {
        ...
        Write-Host "❌ Failed to generate .env: Error occurred" -ForegroundColor Red
    }
}
...
if ($SetupOAuth) {
    ...
    try {
        ...
        Write-Host "❌ OAuth setup failed: Error occurred" -ForegroundColor Red
    }
}
...
if ($Validate) {
    ...
    try {
        ...
        Write-Host "❌ Key validation failed: Error occurred" -ForegroundColor Red
    }
    ...
    try {
        ...
        Write-Host "❌ MCP status check failed: Error occurred" -ForegroundColor Red
    }
    ...
    try {
        ...
        Write-Host "❌ JWT generation failed: Error occurred" -ForegroundColor Red
    }
}
...
# Finalize Heady Academy Environment
Write-Host "`n=== INITIALIZING HEADY ACADEMY ECOSYSTEM ===" -ForegroundColor Magenta

# Define Trinity Paths
$TRINITY_PATHS = @(
    "$HOME\HeadySystems\Heady",
    "$HOME\HeadyConnection\Heady", 
    "$HOME\HeadyConnection_Secondary\Heady"
)
$CONTEXT_PATH = "$HOME\HeadyContext"

# Phase 1: Create Foundation Structure
Write-Host "`n[Phase 1] Establishing Trinity Foundation..." -ForegroundColor Cyan
New-Item -ItemType Directory -Force -Path $TRINITY_PATHS | Out-Null
New-Item -ItemType Directory -Force -Path "$CONTEXT_PATH\Storage", "$CONTEXT_PATH\Logs" | Out-Null
"@{}" | Out-File -FilePath "$CONTEXT_PATH\Logs\global_story.json" -Encoding utf8

# Phase 2: Deploy Security Sentinel
Write-Host "`n[Phase 2] Deploying Heady Sentinel..." -ForegroundColor Yellow
$sentinelScript = @"
import os, time, sys
from watchdog.observers import Observer
from watchdog.events import FileSystemEventHandler

TRINITY = [r'$($TRINITY_PATHS[0].Replace('\','\\'))', r'$($TRINITY_PATHS[1].Replace('\','\\'))', r'$($TRINITY_PATHS[2].Replace('\','\\'))']
LOCK_FILE = r'$env:TEMP\HEADY_IDE_ACTIVE.lock'
